    ...
    <div className="max-w-7xl mx-auto px-6 py-10 flex flex-col md:flex-row md:justify-between md:items-center gap-8">
      <div className="flex flex-col items-center md:items-start gap-4">
        <div className="flex items-center gap-2">
          <span className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-green-500/90 shadow-md">
            ...
            >
              <path
                d="M12 2L2 7l10 5 10-5-10-5zm0 18v-7m0 7c-4.418 0-8-1.79-8-4V7m8 13c4.418 0 8-1.79 8-4V7m-8 5L2 7m10 5 10-5"
                stroke="currentColor"
                strokeWidth="1.5"
                strokeLinecap="round"
        ...
        <div className="flex gap-3 mt-2">
          <a
            href="#"
            aria-label="Twitter"
            className="transition-transform duration-200 hover:scale-110 hover:text-green-300"
          >
          ...
          >
            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
              <path d="M19 0h-14c-2.76 0-5 2.24-5 5v14c0 2.76 2.24 5 5 5h14c2.76 0 5-2.24 5-5v-14c0-2.76-2.24-5-5-5zm-11 19h-3v-10h3v10zm-1.5-11.28c-.966 0-1.75-.784-1.75-1.75s.784-1.75 1.75-1.75 1.75.784 1.75 1.75-.784 1.75-1.75 1.75zm13.5 11.28h-3v-5.604c0-1.337-.025-3.063-1.868-3.063-1.868 0-2.154 1.459-2.154 2.967v5.7h-3v-10h2.881v1.367h.041c.401-.761 1.381-1.563 2.845-1.563 3.043 0 3.604 2.004 3.604 4.609v5.587z" />
            </svg>
          </a>
          <a
            href="#"
      ...
      <div className="flex flex-col items-center gap-2 md:gap-3 md:items-start">
        <div className="flex flex-wrap gap-4 text-sm font-medium">
          <a
            href="#about"
            className="relative group transition-colors duration-200 hover:text-green-300"
          >
            About
            <span className="block h-0.5 max-w-0 group-hover:max-w-full transition-all duration-300 bg-green-400 rounded-full"></span>
          </a>
          <a
            href="#contact"
            className="relative group transition-colors duration-200 hover:text-green-300"
      ...
      <div className="flex flex-col items-center md:items-end gap-3 w-full md:w-auto">
        <form className="flex gap-2 w-full max-w-xs">
          <input
            ...
            className="flex-1 px-3 py-2 rounded-l bg-[#232b39] text-white placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-green-400 transition"
          />
          <button
            type="submit"
            className="px-4 py-2 rounded-r bg-green-500 text-white font-semibold shadow hover:bg-green-400 transition-colors focus:outline-none"
          >
            Subscribe
          </button>
        </form>
        <span className="text-xs text-gray-400">
          Get updates & news (no spam)
        </span>
      </div>
    </div>
